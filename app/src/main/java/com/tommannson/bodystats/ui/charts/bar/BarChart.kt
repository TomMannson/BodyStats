...
import com.github.tehras.charts.line.simpleChartAnimation
...
//      }
            ...
            val barHeigh = height - labelDrawer.requiredAboveBarHeight(this)
            val scopeOfValues = barChartData.maxYValue - barChartData.minYValue

            if (barChartData.maxYValue > 0 != barChartData.minYValue > 0) {
                val linePlace = barDrawableArea.bottom - (calculateShift(
                    calculateZero(
                        barChartData.minYValue,
                        barChartData.maxYValue
                    ),
                    barChartData.minYValue,
                    barChartData.maxYValue
                ) / scopeOfValues) * barHeigh

                drawLine(
                    Color.Black,
                    Offset(
                        barDrawableArea.left, linePlace

                    ),
                    Offset(
                        barDrawableArea.right, linePlace
