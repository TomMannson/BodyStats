...
internal object BarChartUtils {
    ...
    fun BarChartData.forEachWithArea(
        ...
        bars.forEachIndexed { index, bar ->
            ...
            val scopeOfValues = maxYValue - minYValue

            val barArea = Rect(
                left = left + offsetOfBar,
                top = barDrawableArea.bottom - (calculateShift(
                    bar.value,
                    minYValue,
                    maxYValue
                ) / scopeOfValues) * barHeight,
                right = left + widthOfBarArea - offsetOfBar,
                bottom = barDrawableArea.bottom  - (calculateShift(
                    calculateZero(minYValue,maxYValue),
                    minYValue,
                    maxYValue
                ) / scopeOfValues) * barHeight
            )
...
fun calculateZero( min: Float, max: Float): Float {
    if ((min > 0) && (max > 0)) {
        return min
    }
    if ((min < 0) && (max < 0)) {
...
fun calculateShift(value: Float, min: Float, max: Float): Float {
    if ((min > 0) && (max > 0)) {
        return value - min
    }
    if ((min < 0) && (max < 0)) {
        return  Math.abs(min) - Math.abs(value)
    }
    else {
